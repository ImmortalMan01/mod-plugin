...
public class ModerationServiceTest {
    ...
    public void setUp() throws IOException {
        ...
        server.start();
        service = new ModerationService("test", 0.5, 60, java.util.logging.Logger.getAnonymousLogger()) {
            @Override
    ...
    public void testRateLimit() throws Exception {
        service = new ModerationService("test", 0.5, 1, java.util.logging.Logger.getAnonymousLogger()) {
            @Override
            protected String getUrl() { return server.url("/v1/moderations").toString(); }
        };
        server.enqueue(new MockResponse().setBody(response(true, false, 0.7)));
